...
redirect = function( code ) {
	return function (res, data, type, location){
		return res( code || STATUS_CODES[ code ] ).code( code ).type( type ).location( location );
	};
};
 ...
 * @param {Number} status The http status code to set on the response
 **/
exports.temporaryRedirect = function( res, data, type ){
	return res(data || STATUS_CODES[ 307 ] ).code( 307 ).type( type );
};
