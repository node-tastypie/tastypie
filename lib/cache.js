...
Cache = new Class({
	 ...
	 * @returns {Promise<Mixed>} A promise that resolves to the result of the cache operation
	 */
	 ...
	 * @returns {Promise<Mixed>} returns a promise the resolves to the result of the cache operation
	 */
	, set: function set( key, value ){
		return new Promise(function( resolve, reject ){
			this.client.set( key, value, 0, ( err, ...response)=>{
				if( err ){
					return reject( err );
				}
