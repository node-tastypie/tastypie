...
function onPreResponse( request, reply ){
	...
	if( fmt && !ct ){
		return reply(new Boom.UnsupportedFormat( 'Unsupported format: ' + fmt ) );
	}

	format = (fmt && ct) ? mime.determine( request, serializer.types );

	debug('requested format %s', format);

	if( has(query,'callback') && !( has(query, 'format') )  || !format || response.isBoom ){
	  return reply.continue();
	}
