 ...
 * @example
 * // define new acceptable mime type formats
 * serializer = new Serializer({
 * 		content_types:{
 *   		'text/vnd-fakescript':['fake', 'vnd-fake']
 * 		}
...
Serializer = new Class({
	...
	,constructor:function( options ){
		...
		this._mime = new Mime();
		this._mime.define(
			map( this.content_types, function( val, key, obj){
				return toArray( val );
			})
		);
		this._mime.default_type = null;
	}
	...
	,serialize:function( data, format, callback ){
		...
		data           = data || '';
		mime           = this._mime;
		default_format = this.options.defaultFormat;
		...
		desired_format = mime.extension(mime.lookup( '' + format, this.options.defaultFormat) );
	...
	,deserialize: function(data, format, callback ){
		...
		desired_format = mime.extension( mime.lookup( format ) );
