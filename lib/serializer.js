...
Serializer = new Class({
	 ...
	 * @returns {Promise<String>} A promise the resolves the string representation of the input object
	 * @throws SerializationError
	 **/
	,serialize:co.wrap(function* serialize( data = '', format=this.options.defaultFormat ){
		var method_name
		  , desired_format
		  , default_format
		  , mime
		...
		debug(`desired format:${desired_format}`);

		if( typeof this[method_name] === 'function'){
			let res = yield this[method_name]( data ); 
			return res
		}

		throw boom.unsupportedMediaType(`unsupported format ${desired_format}` );
	})
	 ...
	 * @param {String} contentType the content type of the string to be deserialzie. if not specified the defaultFormat option will be used
	 * @returns {Promise<Object>} a promise which resolves a data object containing the deserialized string
	 **/
	,deserialize: co.wrap(function* deserialize(data={}, format = this.options.defaultFormat ){
		var desired_format
		  , method_name
		  , mime
		  ;
		...
		if( typeof this[method_name]  === 'function' ){
			return yield this[method_name]( data );
		}
	 ...
	 * @returns {Promise<String>} returns a promise that resolves to a json string
	 **/
	 ...
	 * @param {String} json valid json string to parse into an object
	 * @returns {Promise<Object>} returns a promise that resolves an object from the parsed json string
	 **/
	,from_json: function( data ){
		return new Promise(( resolve, reject )=>{
			process.nextTick(( err )=>{
				err ? reject( err ) : resolve( JSON.parse( data, null, 2 ) );
			});
		});
	}
	 ...
	 * @returns {Promise<String>} A promise that resolves the xml string created from the input string
	 **/
	,to_xml: function( data ){
		return new Promise(( resolve, reject )=>{
			process.nextTick(( err )=>{
				err ? reject(err) : resolve( data == null ? data : jstoxml.toXML( this.to_jstree( data) , {header:true, indent:" "} ) );
			});
		});
	}
	 ...
	 * @returns {Promise<Object>} A promise the resolves and object parsed from the xml input string
	 **/
			...
			case ARRAY:
				if( this.options.xml.strict ){
					...
					for(let item of data){
						element.push( this.to_jstree( item, name, depth+1 ) );
					}
				} else{
					element = {
						_name: name || OBJECTS
	 ...
	 * @returns {Promise} A promise that resolves a jsonp string
	 **/
