...
Resource = Class(/** @lends module:tastypie.resource.Resource.prototype */{
	...
	,dispatch: function dispatch( action, bundle ){
		...
		if( !method ){
			res.status(500).send(util.format('method %s %s not implemented', requestmethod, req.uri) )
			this.emit('error', new exceptions.NotImplemented( util.format('method %s not implemented', httpmethod) ), req, res )
		}
