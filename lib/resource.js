 ...
 */
Resource = new Class(/** @lends module:tastypie.resource.Resource.prototype */{
	...
	,base_urls: function base_urls(){
		...
		,{
			route: interpolate(urljoin('/', '{{apiname}}', '{{name}}','{id}'), this.options).replace(/\/\//g, "/")
	...
	,dispatch: function dispatch( action, bundle ){
		...
		if( !method ){
			...
			err.req = req;
			err.res = res;
			return res(new Boom.notImplemented( util.format('%s for %s', requestmethod.toUpperCase(), req.path)  ))
	...
	 **/
	,dispatch_list: function list(req, res){

		return this.dispatch('list', this.bundle( req, res ) )
	}
	...
	,post_list: function post_list( bundle ){
		this.deserialize( bundle.req.payload, bundle.req.headers['content-type'], function( err, data ){
			if( err ){
				if( err.code === 1001 ){
					debugger;					
					var reps = bundle.res( new Boom.unsupportedMediaType('Unsupported format: ' + bundle.req.headers['content-type'] ) )
				} else {
					bundle.res( Boom.wrap( err ) );
				}
			} 
			bundle.data = data
			return this.respond( bundle );
	 ...
	 **/
	,buildFilters:function buildFilters( filters ){
		return query;
	}
			...
			return {
				...
				,fields: object.map( this.fields, function( field ){
					...
					return {
						...
						'help_text': field.options.help_text,
	...
	,format: function format( bundle, types ){
		...
		if(fmt && !ct ){
			...
			bundle.res && bundle.res(new Boom.UnsupportedFormat( 'Unsupported format: ' + fmt ) )
		} else if( fmt && ct ){
			return ct;
		}
