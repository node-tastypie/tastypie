...
Resource = Class(/** @lends module:tastypie.resource.Resource.prototype */{
    ...
    ,get_list: function get_list( bundle ){
		...
		this._get_list( bundle,function( e, objects ){
			...
			to_be_serialize[ this.meta.collection ] = to_be_serialize[ this.meta.collection ].map( function( item ){
				return that.full_dehydrate( that.bundle( bundle.req, bundle.res, bundle.next, item ) ) 
			});
	...
	,_get_list: function _get_list( bundle, callback ){
		return fs.readFile(path.resolve("./lib/data.json"), 'utf8', callback )
	}
	...
	 * @param {Bundle} bundle A bundle object
	 * @return Bundle A bundle containing only serializable data
	 **/
	,full_dehydrate: function( bundle ){
		return bundle
	}
	/**
	...
	,respond: function respond( bundle, cls ){
		...
		this.serialize( bundle, format, function(err, data ){
	...
	,format: function format( bundle, types ){
		var f = bundle.req.query && bundle.req.query.format;
		var ct = this.meta.serializer.convertFormat( f )
		if(f && !ct ){
			bundle.res && bundle.res.status( 500 ).send( util.format('unsupported format %s', f ) )
