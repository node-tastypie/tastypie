...
Resource = Class(/** @lends module:tastypie.resource.Resource.prototype */{
	...
	,constructor: function( meta ){
        ...
        this.domain.on('error', this['500'].bind( this ))
	...
	,dispatch: function dispatch( action, bundle ){
		...
		if( !method ){
			var err = new exceptions.NotImplemented( util.format('method %s %s not implemented', requestmethod, req.uri) )
			err.req = req;
			err.res = res;
	...
	,_get_list: function _get_list( bundle, callback ){
		var pathname = path.resolve(__dirname, "data.json")
		debug('reading %s', pathname)
		return fs.readFile(pathname, 'utf8', callback )
	}
	...
	,get_detail: function get_detail( bundle ){
		var obj = this._from_cache('detail', bundle )
	}


	,_from_cache:function _from_cache( type, bundle){
		...
		var obj = this.meta.cache.get(key);

		if(!obj){
			var obj = this.getObject( bundle )
			this.meta.cache.set( key, obj );
		}
	}

	,getObject: function getObject( bundle ){
		var e = new exceptions.NotImplemented("getObject is not implemented")
		e.req = bundle.req
		e.res = bundle.res
		e.next = bundle.next
		this.emit('error',  e)
	...
	,500: function( err ){
		var format = this.format( err, this.meta.serializer.types );
		var req = err.req
		var res = err.res;
		...
		this.serialize( {name:err.name, message:err.message, code:err.code||500}, format, function(serr, data ){
			res
				.status(err.code ||500)
	...
	,deserialize: function deserialize( data, format, callback ){
		return this.meta.serializer.deserialize( data, format, callback )
	}
