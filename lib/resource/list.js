...
List = new Class({
    ...
    , get_list:function* get_list( bundle ){
		...
		out = paginator.page();
		let list = out[ this.options.collection ];
		for( let idx in list ){
			bundle.object = list[idx];
			list[ idx ] = yield this.full_dehydrate( bundle ); 
		}
		bundle.data = out;
		paginator = out = null;
	...
	, post_list:function* post_list( bundle ){
		...
		bundle        = yield this.create_object( bundle )
		...
		bundle.data = yield this.full_dehydrate( bundle )
