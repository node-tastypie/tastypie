...
var path             = require( 'path' )                       // path
  ...
  , dthrottle            = require( 'debug' )( 'tastypie:throttle' ) // debug
	 ...
	 * @example
{
	base_urls: fuinction(){
		return [{
			route:decodeURIComponent('/api/v1/test/{action}'),
			name:"test",
	...
	, throttle: function throttle( bundle ){
		...
		dthrottle("throttle check - request id", request_id)
		throttle = this.options.throttle.toThrottle( request_id )

		if( !throttle ){
			dthrottle("recording throttle %s", request_id )
			this.options.throttle.incr( request_id );
			dthrottle(this.options.throttle._mem)
		}else{
		}
...
var MyResource = new Class({
	...
	, build_schema: function build_schema(){
		...
		return {
			  ...
			  , filtering: this.options.filtering || 'all'
	...
	, respond: function respond( bundle, cls, cb ){
		...
		var that = this;
		debug('requested format: %s', format,bundle.req.headers )
