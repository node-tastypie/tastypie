	 ...
	 * @example
{
	...
	, dispatch: function dispatch( action, bundle ){
		...
		this.domain.run(()=>{
			let result = attempt(method, [bundle], this )
			if( result[0] ){
				this.exception( annotate( result[0], bundle ) );
			}
		});
	}
	...
	, full_dehydrate: function full_dehydrate( obj, bundle, done ){
		...
		async.forEachOf( flds, function eachfield( fld, field, cb){
			...
			fld.dehydrate( obj, ( err, data)=>{
				async_field_dehydrate(err, data, that, field, that[`dehydrate_${field}`], ret, bundle, obj, cb)
			});
		}, function( err ){
