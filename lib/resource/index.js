...
checks = {
	...
	access: function( bundle ){
		...
		if( !this[ method_name ] ){
			...
			return [ new Boom.notImplemented( util.format( "%s is not implemented: %s ", httpmethod, bundle.req.path) ), null ]
		}
	 ...
	 * @example
{
	...
	, exception: function exception( err ){
		...
		if( !reply ){
			debug('no replay object - throwing', err )
			throw err;
		}
	...
	, full_hydrate: function full_hydrate( bundle, done ){
		...
		fields = bundle.req.method.toLowerCase() == 'patch' ? ( bundle.data || {} ) : this.fields;
		bundle = this.hydrate( bundle );
		bundle.object = bundle.object || ( typeof Tpl === 'function' ? new Tpl() : Object.create( Tpl ) );

		async.forEachOf( fields, function(value, fieldname, cb ){
			var methodname
			  , method
			  , attr
			  , field
