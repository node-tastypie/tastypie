...
let domain                = require( 'domain' )                     // domain
  ...
  , { set, merge, get }   = require( 'mout/object' )
  , { clone, isFunction } = require('mout/lang')
...
checks = {
	dispatch: function( bundle ){
		let httpmethod = ( bundle.req.headers['x-method-override'] || bundle.req.method ).toLowerCase();
		let actions    = this.options.allowed[ bundle.action ] || EMPTY_OBJECT;
	...
	access: function( bundle ){
		let httpmethod  = ( bundle.req.headers['x-method-override'] || bundle.req.method ).toLowerCase()
		  ...
		  , method_name = `${httpmethod}_${action}`
		  ;

		if( !this[ method_name ] ){
			return [ Boom.notImplemented(`${httpmethod} is not implemented: ${bundle.req.path}`), null ];
		}
	 ...
	 * @example
{
	 ...
	 **/
	, bundle: function bundle( req, res, data = {}, obj = null ){
		return {
			req: req
			, res: res
			, data: data
	 ...
	 **/
	, deserialize: function deserialize( data, format ){
		return this.options.serializer.deserialize( data, format );
	}
	...
	, dispatch: function dispatch( action, bundle ){
		...
		this.domain.run(()=>{
			co( method.bind( this, bundle ) ).catch((err)=>{
	...
	, exception: function exception( err ){
		...
		this.serialize( {error: err.name, message: err.message, statusCode: code}, format)
			.then(function( data ){
	...
	, filter: function filter( bundle, filters ){
		throw new exceptions.NotImplemented( "filter" );
	}
	...
	, format: function format( bundle, types=this.options.serializer.types ){
		...
		if( fmt && !ct ){
			throw Boom.unsupportedMediaType( `Unsupported format: ${fmt}` );
		} else if( fmt && ct ){
			return ct;
		}
	 ...
	 * @returns {Promise}
	 **/
	, from_cache: co.wrap(function* from_cache( type, bundle ){
		var obj
		  , key = this.cacheKey(
				type
			  , bundle.req.path
	 ...
	 **/
	, full_dehydrate:co.wrap(function* full_dehydrate( bundle ){
		let ret                   = {}
		  , obj                   = bundle.object
		  , { fields, dehydrate } = this
		  ;

		for( let field in fields ){
			let data, methodname, method, fld;
	 ...
	 * @return {Bundle}
	 **/
	, full_hydrate:function* full_hydrate( bundle ){
		...
		bundle.object = bundle.object || ( isFunction(Tpl) ? new Tpl() : Object.create( Tpl ) );

		for( let name in flds ){
			let methodname
			  , attr
			  , field
			  ;
			...
			method = this[ `hydrate_${name}` ];
			bundle = isFunction( bundle ) ? method( bundle ) : bundle;
			attr   = field.options.attribute;

			if( attr ){
				let value = yield field.hydrate( bundle );
	...
	, respond: function respond( bundle, cls, location, cb ){
		...
		this.serialize( bundle.data, format ).then(function( data ){
			let reply = cls( bundle.res, data || null, format, location );
			let mutable = mutableMethods.indexOf( bundle.req.method.toLowerCase() ) !== -1;
			this.modified = mutable ? (new Date()).toUTCString() : this.modified;
			this.modified && reply.header('Last-Modified', this.modified );
			cb && cb(null,reply)
		}.bind( this ), this.exception );
	}
	 ...
	 * @requires {Promise}
	 **/
	, serialize: function serialize( data, format ){
		return this.options.serializer.serialize( data, format );
	}
