...
ApiField = new Class({
	...
	, hydrate: function hydrate( bundle, cb ){
		...
		if( hasOwn(bundle.data, name ) ){
			return cb( null, bundle.data[ name ] );
		}
		...
		if( attr && attr in obj ){
			return cb( null, obj[ attr ] );
		} else if ( name in obj ){
			return cb( null, obj[ name ] );
		} else if( this.options.hasOwnProperty( 'default' ) && !isUndefined( this.options.default ) ){
			return cb( null, isFunction( this.default ) ? this.default( bundle.data ) : this.default );
	...
	, dehydrate: function dehydrate( obj, cb ){
		...
		return process.nextTick(cb, null, this.convert( current ))
	}
