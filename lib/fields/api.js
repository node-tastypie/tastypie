...
ApiField = new Class({
	...
	, hydrate: function hydrate( bundle ){
		...
		return new Promise(function(resolve, reject){
			...
			if( hasOwn(bundle.data, name ) ){
				return resolve( bundle.data[ name ] );
			}
			...
			if( attr && attr in obj ){
				...
				value =  obj[ name ];
			} else if( this.options.hasOwnProperty( 'default' ) && !isUndefined( this.options.default ) ){
				value =  isFunction( this.default ) ? this.default( bundle.data ) : this.default;
			} else if( this.options.nullable ){
				value = null;
			} else {
				return reject( boom.create(400, `Field ${name} has no data, default value and is not nullable`) );
			}
	...
	, dehydrate: function dehydrate( obj ){
		return new Promise(function( resolve, reject ){
			...
			setImmediate(resolve, this.convert( current ) )
		}.bind( this ))
	}
